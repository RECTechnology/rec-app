...
void main() {
  ...
  group('Get remaining times until next open/close', () {
    ...
    test('returns next closing time for just one range', () async {
      final now = DateTime.now();
      final weekday = now.weekday - 1; // remove one as weekday starts at 1
      final days = List.generate(7, (index) => ScheduleDay());
      final formatter = DateFormat.Hm();
      ...
      // Create closed schedule for today
      days[weekday] = ScheduleDay(
        firstOpen: formatter.format(now.subtract(Duration(hours: 2))),
        firstClose: formatter.format(now.add(Duration(hours: 8))),
      );
      ...
      final nextOpen = schedule.getNextClosingTime(now);
      final expectedNextClose = now.add(Duration(hours: 8));

      expect(nextOpen?.day, expectedNextClose.day);
      expect(nextOpen?.month, expectedNextClose.month);
      expect(nextOpen?.year, expectedNextClose.year);
      expect(nextOpen?.hour, expectedNextClose.hour);
      expect(nextOpen?.minute, expectedNextClose.minute);
    });

    test('returns next closing time for just 2 ranges', () async {
      final now = DateTime.now();
      final weekday = now.weekday - 1; // remove one as weekday starts at 1
      final days = List.generate(7, (index) => ScheduleDay());
      final formatter = DateFormat.Hm();
      ...
      // Create closed schedule for today
      days[weekday] = ScheduleDay(
        firstOpen: formatter.format(now.subtract(Duration(hours: 4))),
        firstClose: formatter.format(now.subtract(Duration(hours: 2))),
        secondOpen: formatter.format(now.subtract(Duration(hours: 2))),
        secondClose: formatter.format(now.add(Duration(hours: 8))),
      );
