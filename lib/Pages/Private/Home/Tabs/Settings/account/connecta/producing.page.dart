...
class ConnectaProducingPage extends StatefulWidget {
  ConnectaProducingPage({Key? key}) : super(key: key);

  @override
  State<ConnectaProducingPage> createState() => _ConnectaProducingPageState();
}

class _ConnectaProducingPageState extends State<ConnectaProducingPage> {
  List<Product> selectedProducts = [];

  @override
  Widget build(BuildContext context) {
    final account = UserState.of(context).account;
    final products = account?.producingProducts ?? [];
    return Scaffold(
      appBar: EmptyAppBar(context, title: 'PRODUCING'),
  ...
  void _removeProducts(selectedProducts) async {
    ...
    for (var product in selectedProducts) {
      await provider.removeProduct(userState.account!.id!, product.id, ProductType.producing);
    }
    ...
    showDialog(
      ...
      builder: (_) => Dialog(
        ...
        child: AddProductContent(
          addProduct: (value) {
            return provider.addProduct(value, ProductType.producing);
          },
        ),
      ),
    );
